...
# Objective: minimize sum of absolute flows weighted by costs
# This is convex since it's a sum of compositions of convex functions
objective = cp.Minimize(cp.sum(cp.multiply(costs, cp.abs(flow))))
...
# Solve with different solvers (try ECOS, SCS, or OSQP)
# ECOS is good for general convex problems
try:
...
if prob.status in ["optimal", "optimal_inaccurate"]:
    ...
    df_res.to_csv("min_flow_solution_cvxpy.csv", index=False)
    
    # Write summary
    with open("min_flow_summary_cvxpy.txt", "w") as f:
        f.write(f"Status: {prob.status}\n")
        f.write(f"Total cost: {prob.value}\n")
        f.write(f"Solver: CVXPY\n")
    
    print(f"\nTotal cost: {prob.value}")
    print("Saved min_flow_solution_cvxpy.csv and min_flow_summary_cvxpy.txt.")
else:
